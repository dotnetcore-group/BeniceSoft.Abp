...
public class ExceptionHandlingMiddleware : IMiddleware, ITransientDependency
{
    ...
    private static async Task HandleRemoteServiceErrorResponse(HttpContext context, Exception exception)
    {
        ...
        await context.Response.WriteAsync(JsonUtils.Serialize(remoteServiceErrorResponse) 
            ?? string.Empty)
    ...
    private static ResponseResult WarpExceptionToJsonResult(Exception exception)
    {
        return exception switch
        {
            UserFriendlyException userFriendlyException
                => new(HttpStatusCode.BadRequest, userFriendlyException.Message),

            NoAuthorizationException
                => new(HttpStatusCode.Unauthorized, NoAuthorizationException.DefaultMessage),
            ...
            SynchronizationLockException
                => new(HttpStatusCode.Locked, "资源已被占用，请稍候再试"),

            AbpRemoteCallException remoteCallException
                => new(HttpStatusCode.ServiceUnavailable, $"远程服务不可用({remoteCallException.HttpStatusCode})"),
